...
# Path to Excel file
EXCEL_PATH = '/Users/heilman/Desktop/Conway SC Leapfrog Historical Measure Analysis 2021-2025.xlsx'

def parse_excel():
    """Parse the Excel file and extract all data"""
    wb = openpyxl.load_workbook(EXCEL_PATH, data_only=True)
    ws = wb['Measure Scores']

    measures = []

    # Parse each measure (rows 4 onwards)
    for row in range(4, ws.max_row + 1):
        domain = ws.cell(row, 1).value
        measure_id = ws.cell(row, 2).value
        measure_name = ws.cell(row, 3).value

        if not measure_id or measure_id == 'Measure ID':
            continue
        ...
        measure = {
            ...
            'summary': {
                'conwayMean': ws.cell(row, 19).value,
                'nationalMean': ws.cell(row, 20).value,
                'conwaySlope': ws.cell(row, 21).value,
                'nationalSlope': ws.cell(row, 22).value,
                'weight': ws.cell(row, 23).value
            },
            'forecasts': {
                '2026': {
                    'conwayResult': ws.cell(row, 24).value,
                    'nationalMean': ws.cell(row, 25).value
                },
                '2027': {
                    'conwayResult': ws.cell(row, 26).value,
                    'nationalMean': ws.cell(row, 27).value
                }
            }
        }
...
def calculate_measure_group_scores(measures):
    ...
    for measure in measures:
        ...
        for period, data in measure['periods'].items():
            ...
            if zscore is not None and zscore != '*' and zscore != 'Not Included':
                try:
    ...
    periods = ['2023 Spring', '2023 Fall', '2024 Spring', '2024 Fall', '2025 Spring']
...
def generate_typescript_output(measures):
    ...
    output.append('// Source: Conway SC Leapfrog Historical Measure Analysis 2021-2025.xlsx')
    output.append('// Generated: ' + str(os.popen('date').read().strip()))
    output.append('')
